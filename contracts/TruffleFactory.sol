...
contract TruffleFactory is ERC721, Ownable{
    ...
    function _triggerCooldown(Truffle storage _truffle) internal {//hàm kich hoat cooldown
        ...
        if(_truffle.rarity >= 75){
            for(uint8 i = 0; i <= 20; i++){
                if(i == levels){
                    _truffle.readyTime = uint32(block.timestamp + cooldowns[0] - (lostTime * i));
                    break;
                }
            }
            ...
            for(uint8 i = 0; i <= 20; i++){
                if(i == levels){
                    _truffle.readyTime = uint32(block.timestamp + cooldowns[1] - (lostTime * i));
                    break;
                }
            }
            ...
            for(uint8 i = 0; i <= 20; i++){
                if(i == levels){
                    _truffle.readyTime = uint32(block.timestamp + cooldowns[2] - (lostTime * i));
                    break;
                }
            }
        }
   ...
   function BreedAndMultiply(uint _truffleId, uint _targetDna) public payable {
